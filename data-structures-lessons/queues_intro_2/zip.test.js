...
const zip = require(resolve(__dirname, 'zip'));

describe('zip(queue):', function () {
    this.bail(false);
    context('when queue is empty,', () => {
        it('should do nothing', () => {
            const queue = new Queue();

            expect(() => zip(queue)).to.not.throw();
            expect(queue.peek()).to.be.undefined;
        });
    });

    context('when queue is not empty,', () => {
        it('should zip up the first and second halves of the values in queue into queue, without changing the front value of queue', () => {
            const queue = new Queue();
            queue.enqueue(0);
            queue.enqueue(1);
            queue.enqueue(2);
