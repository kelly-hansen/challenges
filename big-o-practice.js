...
// Big-O for getUnique3(n): O(n^2)
function getUnique3(values) {
  const uniqueValues = new Set();  // 1. O(1) * 2. O(1) = 3. O(1)

  for (
    let valueIndex = 0;  // 1. O(1) * 2. O(1) = 3. O(1)
    valueIndex < values.length;  // 1. O(1) * 2. O(n) = 3. O(n)
    valueIndex++  // 1. O(n) * 2. O(n) = 3. O(n)
  ) {
    const value = values[valueIndex];  // 1. O(1) * 2. O(n) = 3. O(n)
    if (!uniqueValues.has(value)) {  // 1. O(n) * 2. O(n) = 3. O(n^2)
      uniqueValues.add(value);  // 1. O(n) * 2. O(n) = 3. O(n^2)
    }
  }

  return Array.from(uniqueValues);  // 1. O(n) * 2. O(1) = 3. O(n)
}
